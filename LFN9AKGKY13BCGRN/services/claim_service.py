...
def _filters_cache_key(filters: dict | None) -> str:
    """Stable cache key for Snowflake-filtered recent-claims queries."""
    ...
    filters = filters or {}
    normalized = {key: normalize(filters.get(key)) for key in sorted(filters)}
    return json.dumps(normalized, sort_keys=True)


@st.cache_data(ttl=300, show_spinner="Loading claims...")
def _get_filtered_recent_claims_cached(_session, cache_scope: str, filters_key: str, filters: dict, page: int, page_size: int) -> pd.DataFrame:
    del cache_scope, filters_key
    started = perf_counter()
    df = claims_repository.get_filtered_recent_claims(_session, filters, page, page_size)
    logger.info("recent_claims_load_ms=%d page=%s page_size=%s rows=%d filters=%s", int((perf_counter() - started) * 1000), page, page_size, len(df), filters)
    return df


@st.cache_data(ttl=300, show_spinner=False)
def _get_filtered_claims_count_cached(_session, cache_scope: str, filters_key: str, filters: dict) -> int:
    del cache_scope, filters_key
    started = perf_counter()
    count = claims_repository.get_filtered_claims_count(_session, filters)
    logger.info("recent_claims_count_load_ms=%d count=%d filters=%s", int((perf_counter() - started) * 1000), count, filters)
    return count
...
def get_filtered_recent_claims(session, filters: dict | None, page: int, page_size: int) -> pd.DataFrame:
    ...
    if _in_streamlit_runtime():
        return _get_filtered_recent_claims_cached(session, _claims_cache_scope(), _filters_cache_key(normalized_filters), normalized_filters, int(page), int(page_size))
    return claims_repository.get_filtered_recent_claims(session, normalized_filters, page, page_size)


def get_filtered_claims_count(session, filters: dict | None) -> int:
    normalized_filters = filters or {}
    if _in_streamlit_runtime():
        return _get_filtered_claims_count_cached(session, _claims_cache_scope(), _filters_cache_key(normalized_filters), normalized_filters)
    return claims_repository.get_filtered_claims_count(session, normalized_filters)


def get_available_claim_statuses(session) -> list[str]:
    if _in_streamlit_runtime():
        return _get_available_claim_statuses_cached(session, _claims_cache_scope())
    return claims_repository.get_available_claim_statuses(session)


def get_available_claim_types(session) -> list[str]:
    if _in_streamlit_runtime():
        return _get_available_claim_types_cached(session, _claims_cache_scope())
    return claims_repository.get_available_claim_types(session)



@st.cache_data(ttl=300, show_spinner=False)
def _get_claim_detail_by_id_cached(_session, cache_scope: str, claim_id: str) -> dict[str, Any] | None:
    del cache_scope
    return _get_claim_detail_by_id_uncached(_session, claim_id)


@st.cache_data(ttl=300, show_spinner=False)
def _get_current_mfq_answers_by_claim_id_cached(_session, cache_scope: str, claim_id: str) -> pd.DataFrame:
    del cache_scope
    return _get_current_mfq_answers_by_claim_id_uncached(_session, claim_id)


@st.cache_data(ttl=300, show_spinner=False)
def _get_claim_history_by_claim_id_cached(_session, cache_scope: str, claim_id: str) -> pd.DataFrame:
    del cache_scope
    return _get_claim_history_by_claim_id_uncached(_session, claim_id)


@st.cache_data(ttl=300, show_spinner=False)
def _get_claim_documents_by_claim_id_cached(_session, cache_scope: str, claim_id: str) -> pd.DataFrame:
    del cache_scope
    return _get_claim_documents_by_claim_id_uncached(_session, claim_id)


@st.cache_data(ttl=300, show_spinner=False)
def _get_claim_summaries_by_claim_id_cached(_session, cache_scope: str, claim_id: str, tab_name: str) -> dict[str, str]:
    # tab_name deliberately participates in the cache key so each lazy tab has its own cached entry.
    del cache_scope, tab_name
    return _get_claim_summaries_by_claim_id_uncached(_session, claim_id)
...
def _get_claim_detail_by_id_uncached(session, claim_id: str) -> dict[str, Any] | None:
    started = perf_counter()
...
def get_claim_detail_by_id(session, claim_id: str) -> dict[str, Any] | None:
    if _in_streamlit_runtime():
        return _get_claim_detail_by_id_cached(session, _claims_cache_scope(), str(claim_id))
    return _get_claim_detail_by_id_uncached(session, claim_id)
...
def _get_current_mfq_answers_by_claim_id_uncached(session, claim_id: str) -> pd.DataFrame:
    started = perf_counter()
...
def get_current_mfq_answers_by_claim_id(session, claim_id: str) -> pd.DataFrame:
    if _in_streamlit_runtime():
        return _get_current_mfq_answers_by_claim_id_cached(session, _claims_cache_scope(), str(claim_id))
    return _get_current_mfq_answers_by_claim_id_uncached(session, claim_id)
...
def _get_claim_history_by_claim_id_uncached(session, claim_id: str) -> pd.DataFrame:
    started = perf_counter()
...
def get_claim_history_by_claim_id(session, claim_id: str) -> pd.DataFrame:
    if _in_streamlit_runtime():
        return _get_claim_history_by_claim_id_cached(session, _claims_cache_scope(), str(claim_id))
    return _get_claim_history_by_claim_id_uncached(session, claim_id)


def _get_claim_documents_by_claim_id_uncached(session, claim_id: str) -> pd.DataFrame:
    started = perf_counter()
...
def get_claim_documents_by_claim_id(session, claim_id: str) -> pd.DataFrame:
    if _in_streamlit_runtime():
        return _get_claim_documents_by_claim_id_cached(session, _claims_cache_scope(), str(claim_id))
    return _get_claim_documents_by_claim_id_uncached(session, claim_id)


def _get_claim_summaries_by_claim_id_uncached(session, claim_id: str) -> dict[str, str]:
    started = perf_counter()
...
def get_claim_summaries_by_claim_id(session, claim_id: str, tab_name: str = "summary") -> dict[str, str]:
    if _in_streamlit_runtime():
        return _get_claim_summaries_by_claim_id_cached(session, _claims_cache_scope(), str(claim_id), str(tab_name))
    return _get_claim_summaries_by_claim_id_uncached(session, claim_id)
